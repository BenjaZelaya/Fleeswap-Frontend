...
export default function SolicitudEnviadaCard({ solicitud, onUpdateSuccess }) {
  ...
  const handleCancelar = () => {
    ...
    setModalConfig({
      title: isPending ? '¿Retirar propuesta?' : '¿Cancelar intercambio?',
      message: isPending 
        ? '¿Estás seguro de que deseas retirar esta propuesta de intercambio? Podrás proponer otra más adelante si lo deseás.' 
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          toast.success(isPending ? 'Propuesta retirada con éxito.' : 'El intercambio ha sido cancelado exitosamente.')
          if (onUpdateSuccess) onUpdateSuccess()
        } catch (error) {
          const errorMsg = error.response?.data?.message || 'Hubo un error al intentar procesar la cancelación.'
