...
function SolicitudCard({ solicitud }) {
  const { solicitante, publicacionOferta, publicacionDestino, estado, monto, createdAt } = solicitud

  const ownerInitial = solicitante?.nombre?.[0]?.toUpperCase() ?? '?'
  const ownerName = [solicitante?.nombre, solicitante?.apellido].filter(Boolean).join(' ') || 'Usuario'
  ...
  return (
    ...
    >
      {/* Header — solicitante + fecha + estado */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-50 bg-slate-50/40">
        <Link
          to={`/profile/${solicitante?._id ?? solicitante?.id}`}
          className="flex items-center gap-2.5 group min-w-0"
        >
          {solicitante?.photo ? (
            <img
              src={solicitante.photo}
              alt={ownerName}
              className="w-9 h-9 rounded-full object-cover border-2 border-white shadow-sm shrink-0"
            />
          ) : (
            <div className="w-9 h-9 rounded-full bg-brand-accent flex items-center justify-center text-white text-sm font-bold shrink-0 border-2 border-white shadow-sm">
              {ownerInitial}
            </div>
          )}
...
function EmptyState({ filtro }) {
  const mensaje = filtro === 'all'
    ? 'Aún no recibiste propuestas de intercambio.'
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-slate-400 mt-1 max-w-xs">
        Seguí publicando objetos para atraer más interesados.
      </p>
      <Link
        to="/explore"
...
export default function SolicitudesRecibidas() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        logError('Error fetching solicitudes recibidas:', err)
        if (!cancelled) setError('No se pudieron cargar las solicitudes. Intentá de nuevo.')
      } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <Seo
        title="Solicitudes recibidas · Fleeswap"
        description="Revisá las propuestas de intercambio que recibiste de otros usuarios."
      />
      ...
      <div className="flex items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Propuestas de intercambio hacia tus publicaciones.</p>
        </div>
        <Link
        ...
        {ESTADOS.map((e) => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-semibold transition-colors ${activo
                  ? 'bg-brand text-white'
        ...
        <AnimatePresence mode="popLayout">
          <motion.div key={filtro} variants={listVariants} initial="hidden" animate="show" className="space-y-4">
            {listadoFiltrado.map((s) => (
              <SolicitudCard key={s._id ?? s.id} solicitud={s} />
            ))}
          </motion.div>
        </AnimatePresence>
      )}
