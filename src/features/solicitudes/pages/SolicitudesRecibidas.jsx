...
function EmptyState({ filtro }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center py-20 text-center">
      ...
      <p className="text-slate-700 font-semibold">{filtro === 'all' ? 'Aún no recibiste propuestas.' : `No tenés solicitudes ${label}.`}</p>
...
export default function SolicitudesRecibidas() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <Seo title="Solicitudes recibidas · Fleeswap" description="Revisá las propuestas de intercambio que recibiste." />
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Solicitudes recibidas</h1>
          <p className="text-sm text-slate-400 mt-0.5">Propuestas hacia tus publicaciones.</p>
        </div>
      ...
      {loading ? (
        ...
        <AnimatePresence mode="popLayout">
          <motion.div key={filtro} variants={listVariants} initial="hidden" animate="show" className="space-y-4">
            {filtradas.map((s) => <SolicitudCard key={s._id ?? s.id} solicitud={s} />)}
          </motion.div>
        </AnimatePresence>
      )}
    </div>
